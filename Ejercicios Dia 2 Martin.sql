...
SELECT FIRST_NAME,LAST_NAME,SALARY
FROM employees
WHERE SALARY NOT IN (10000,15000) AND (DEPARTMENT_ID=30 OR DEPARTMENT_ID=100)
ORDER BY SALARY
...
FROM employees
HAVING INSTR(FIRST_NAME,'b') AND INSTR(FIRST_NAME,'c')
...
FROM employees
WHERE (JOB_ID='IT_PROG' OR JOB_ID='ST_CLERK') AND SALARY <> 4500 AND SALARY <> 10000 AND SALARY <> 15000
ORDER BY SALARY
